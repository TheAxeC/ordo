...
1. `.agents/plan.yaml` at the repository root: the only place a project specific lives (the roadmap, the verification page, the rules file, the standards, the ledger root, the archive root, the worktree root, the review cadence). A repository that holds several projects lists them under `projects:`, and `<entry>` is then `<project>/<entry>`. No file, no run: the skill stops and says the file is missing.
...
- `orchestrator-state.md` from `templates/orchestrator-state.md`: the configuration block filled in from `plan.yaml` (the verification commands copied from the page, the rules file, the standards, the worktree root, the worker, the review cadence), the dispatch block empty, the open items empty, the position naming the first step.
