...
- 1c A brief checked against itself before dispatch (ruling J (a)): `spec`'s `templates/brief.md` gains a "Cases" section (every must-pass and must-refuse example the brief lists, in one list), a "Decisions checked against the cases" section (the command that ran each decision over the cases, a prototype of a check rule or the existing behaviour run first, and its output, saved with the step's run files) and a "Paths this step writes" section (one path per line, a shared document with its line range); `spec`'s Steps run both checks before the preparation commit, fix a contradiction before dispatch or stop when the fix changes the scope, and refuse a path shared with the brief of a step in the dispatch block, naming both steps; `refute` reruns the cases command and checks each case; `plan-orchestration`'s "Two steps in flight" names the path section and its check; the path check is a script in `spec`'s `templates/` with a test red under each revert (1 commit)
