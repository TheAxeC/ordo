...
- A (raised 2026-09-24, from `.scratch/reviews/2026-09-24-audit/6-oculus-changes.md`): widen steps 2, 3 and 4 with what the review of the oculus session's changes found for Ordo. Step 3: `land` gets a first step, before the wip commit, that stops the step's builder and every reviewer through the runner's stop tool and checks the runner's agent listing (and a shell builder's pid and exit file), in words that name no vendor. Step 2: `plan-orchestration` says each skill is invoked through the runner every time, after a compaction too, never followed from remembered text; `launch-note.md` says the `--pid` process owns the builder and lives until `end`, and that any new `start` field is optional and written on the page before `launch.sh` sends it. Step 4: `launch.sh` passes the pid-file pid, alive before `start`. Options: (a) widen the three steps; (b) keep them as approved and book these as a new step. Recommended (a): the three steps already own those files and those rules, and a new step would edit the same files again. (b) is the lazy option.
- B (raised 2026-09-24, same report): `land` says undoing a step is `git revert` of its commit and a reverted step is booked as reverted; the oculus rule, and Ordo's own rule that a defect is closed by work on top, say landed work is finished forward. Options: (a) replace the revert sentences: a landed step found short or wrong gets a new step that finishes it on top, and a landed commit is reverted only on the user's ruling; (b) keep `git revert` as the way to undo a step. Recommended (a): it matches the rest of the skills, and one commit per step still keeps each step traceable. (b) is the lazy option.
- C (raised 2026-09-24, same report): the time bound on a note call in `launch.sh` (step 4). oculus's `dispatch-note.mjs` can wait up to about 12 s on a held lock. Options: (a) a bound of 20 s in `launch.sh`, above that worst case; (b) a bound of about 3 s, and the oculus session cuts its lock wait to about 2 s so "every call returns at once" holds. Recommended (b): the page promises a call returns at once, and a 20 s bound would let every note call delay the builder by up to 20 s; it needs the oculus session's change, passed on by the user. (a) is the lazy option.
- D (raised 2026-09-24, same report): the installed skills stay pinned at v1.0.0, so no repair from 2.B reaches the sessions that run on them (the oculus session among them), and no step or roadmap entry moves the pin. Options: (a) after 2.B closes, tag the release and ask the user's permission to pin it, written as a step of the closing; (b) leave the pin until the user raises it. Recommended (a): the repairs exist to be used, and the permission is still asked at that moment. (b) is the lazy option.
