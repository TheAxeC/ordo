...
- `~/.claude/CLAUDE.md` and the rules under `~/.claude/rules/`. The ones that bite here: a named thing is its whole (a skill is its folder, templates included); never take the lazy option; no claim about state without a command in the same turn; plain prose, ASCII, no history in a rule file or a comment.
...
- Ledger: `.scratch/2-a-launch-notes-for-builders-run-as-their-own-process/`, with `agents/briefs/` and `agents/reviews/`.
- The request this entry answers: the oculus migration session's note (the interface and the recipes' wrapped forms), copied into the rulings of `plan.md`.
