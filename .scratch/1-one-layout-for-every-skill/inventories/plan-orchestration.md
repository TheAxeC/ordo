...
| 3 | The same skill runs a code tool, a research project or a manuscript on either harness, with either as the worker | The two tiers, and the harnesses 6 |
...
| 10 | It carries no project name and no vendor name; those are in .agents/plan.yaml and the ledger | Rules 1 |
| 14 | Two tiers of model take part, neither tied to one vendor | The two tiers, and the harnesses 1 |
| 14 | The orchestrator runs on a top-tier model (Claude Fable or GPT Astra): reads, decides, invokes, lands, books | The two tiers, and the harnesses 2 |
| 14 | The orchestrator never writes step code beyond a fix at landing, unless executor is inline | The two tiers, and the harnesses 2 |
| 14 | The builder runs on the working tier (Claude Opus or GPT Sol), one per step, in the worktree, under the brief and the rules file | The two tiers, and the harnesses 3 |
| 14 | The reviewer is whatever reviewer: names | The two tiers, and the harnesses 4 |
| 14 | Any combination, chosen per step; a new one booked in the rulings with what decides it and measured by its usage row | The two tiers, and the harnesses 5 |
...
| 30 | A stop it raises goes to the user; the loop moves on or pauses | Steps 3 |
...
| 31 | The prompt states the worktree, the no-git rule, what is never touched, the reading order, every requirement, the report path and shape | Steps 4 |
| 31 | The builder never runs git and never writes into the ledger | Steps 4 |
...
| 33 | Save the report where the builder could not | Steps 6 |
...
| 35 | Small things are fixed at landing, the last run's findings included | Steps 8 |
| 35 | The rest is booked as its own step, never sent back | Steps 8 |
| 35 | The one exception: one round beyond the cap for a red verification command or an unbuilt acceptance item too large for landing | Steps 8 |
| 35 | A new finding never earns that round | Steps 8 |
...
| 37 | The loop ends at a stop, a pause or nothing unblocked | Steps 10 |
| 37 | The final message lists every landed step, the open items and the booked count | Steps 10 |
...
| 45 | A rule already written that keeps being broken is not rewritten; what is proposed is a check | The recurring-findings pass 3 |
| 45 | Not rewritten, the same rule | Anti-patterns 6 |
...
| 60-62 | The claude -p launch command | Launching a builder 2 |
| 65 | Codex builder: codex exec from a shell, detached from the command timeout | Launching a builder 3 |
| 68-71 | The codex exec launch command | Launching a builder 3 |
| 74 | -C is the working root and -s workspace-write confines writes | Launching a builder 4 |
| 74 | The network setting is passed when the verification commands bind a port | Launching a builder 5 |
| 74 | -o writes the final message; --json streams the event log with the usage | Launching a builder 6 |
| 74 | Not --ephemeral, so the rollout is the builder's transcript | Launching a builder 7 |
| 74 | The shell tool caps a command at ten minutes, so the launch is detached and a monitor watches the exit file | Launching a builder 8 |
| 74 | Codex settings in the repository's .codex/; the orchestrator never edits a user-level file | Launching a builder 9 |
...
| 78 | Everything else is closed in the open step, the repair round or the landing | What earns a step of its own 2 |
...
| 82 | A stop is for a decision that is the user's: an unnamed shape, a wrong premise, a red check, a rule clash | Stops |
| 82 | Fixing a defect in what was asked is never a stop, whatever it makes visible | Stops 5 |
| 82 | A miss inside a brief is closed in the round or at landing, or booked, never handed back as a gap | Anti-patterns 3 |
| 82 | A fix of a defect in delivered work needs no yes | Rules 2 |
| 82 | A stop is booked in the open items at once | Stops 6 |
| 82 | A stop is repeated in every report until ruled | Stops 7 |
| 82 | The turn ends only when nothing unblocked is left or at a pause | Steps 10 |
| 82 | A pause holds until the user lifts it | Stops 9 |
| 82 | A stop goes in one message with options inside the rules and one recommendation | Stops 8 |
| 82 | An option that breaks a written rule is not mentioned | Anti-patterns 5 |
| 86 | Every report opens with the open items | Reports 1 |
| 86 | The open items hold only stops and recurring-findings proposals | Reports 2 |
| 86 | A finding that needs no ruling is a step in the booked list; a report names its count and steps | Reports 3 |
| 86 | The closed list is a log that no report carries | Reports 4 |
| 86 | NOT DONE first, then the DONE / NOT DONE ledger naming the command per row | Reports 5 |
