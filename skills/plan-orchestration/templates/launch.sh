...
usage() {
    printf 'Usage: %s claude --cwd <dir> --model <model> --prompt <file> --report <file> --stderr <file> --exit <file> --pid <file> [--note <command> --id <file> --label <step> --parent <session id>]\n' "$self" >&2
    printf '       %s codex --cwd <dir> --model <model> --prompt <file> --report <file> --stderr <file> --exit <file> --pid <file> --events <file> --effort <effort> [--network] [--note <command> --id <file> --label <step> --parent <session id>]\n' "$self" >&2
...
opt_note='' opt_id='' opt_label='' opt_parent=''
opt_path=''
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --network)
            ...
            shift
            ;;
        --cwd | --model | --prompt | --report | --stderr | --exit | --pid | --events | --effort | --note | --id | --label | --parent)
...
run_claude() {
    cd "$opt_cwd" || return
    claude -p --model "$opt_model" --permission-mode acceptEdits \
        --output-format json <"$opt_prompt" >"$opt_report" 2>"$opt_stderr"
}

run_codex() {
    if [ "$opt_network" -eq 1 ]; then
        codex exec -C "$opt_cwd" -s workspace-write \
            -c "sandbox_workspace_write.network_access=true" -c "model_reasoning_effort=\"$opt_effort\"" -m "$opt_model" \
            -o "$opt_report" --json - <"$opt_prompt" >"$opt_events" 2>"$opt_stderr"
    else
        codex exec -C "$opt_cwd" -s workspace-write \
            -c "model_reasoning_effort=\"$opt_effort\"" -m "$opt_model" \
            -o "$opt_report" --json - <"$opt_prompt" >"$opt_events" 2>"$opt_stderr"
    fi
}
...
case "$mode" in
    ...
    transcript)
        for pair in "cwd:$opt_cwd" "model:$opt_model" "prompt:$opt_prompt" "report:$opt_report" "stderr:$opt_stderr" \
            "exit:$opt_exit" "pid:$opt_pid" "events:$opt_events" "effort:$opt_effort" "label:$opt_label" "parent:$opt_parent"; do
