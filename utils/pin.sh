...
# A skill is a top-level folder of the tag that holds SKILL.md. A link into the pinned worktree whose
...
check_links() {
    ...
    for skill in $(skills_of "$stable"); do
        for dir in $skill_dirs; do
            ...
            if [ "$target" != "$stable/$skill" ]; then
                printf 'pin: %s/%s does not link to %s/%s\n' "$dir" "$skill" "$stable" "$skill" >&2
                problems=$((problems + 1))
            fi
        done
    done
...
tag_skills=$(git -C "$repo" ls-tree -r --name-only "$tag" | sed -n 's#^\([^/]*\)/SKILL\.md$#\1#p')
...
for dir in $skill_dirs; do
    mkdir -p "$dir"
    for skill in $tag_skills; do
        ln -sfn "$stable/$skill" "$dir/$skill"
    done
    for link in "$dir"/*; do
        [ -L "$link" ] || continue
        case "$(readlink "$link")" in
            "$stable"/*|"$repo"/*)
                [ -f "$stable/$(basename "$link")/SKILL.md" ] || rm "$link"
                ;;
        esac
    done
done
