...
dispatch:
  ...
  worker_id: <pid while running; the session, rollout or agent id once known>
  landing: not-started                     # not-started | cherry-picking | verified
```

An orchestrator that finds this block on resumption checks the exit file, the process and `landing` before anything else: a worker still running is waited for; a finished one resumes at step 7, the diff read, and then verification, never at the landing; a package at `cherry-picking` is checked on main (`git status`, `git diff --cached`) before anything is applied again, so a cherry-pick is never applied twice; a dead worker (no exit file, no process) is reported to the user with what its logs hold, never silently relaunched.
...
7. On the report: save it to the path the dispatch block names (a Claude worker's final message is written there by the orchestrator, since another harness cannot retrieve it from the runner), record the worker's identity in the block (its pid while it runs, its session or rollout id once known), then read the whole diff. The report is a lead, not a fact.
...
10. Repair round: a miss found by the read, the reviewer or the checks that stays inside the brief (an acceptance item not built, a test not written, a convention not met, a red check) goes back to the same worker once, with the list of misses, in the same worktree; the round is counted in the usage row. A miss that would change the scope, a requirement, a public shape or an established decision goes to the user instead. A repaired diff goes back through steps 7 to 9 in full: the whole diff read again, the reviewer again when one is named, the `verify` commands again; nothing is booked on a repair that was not re-read and re-run. After one repair round, what is still short is either fixed by the orchestrator at landing (counted as a fix, and re-verified) or reported to the user with the misses sorted by cause: the orchestrator's configuration, the brief, a rule nobody wrote, the worker.
11. Land it: in the worktree `git add -A <the tool's directory> && git commit -q -m wip`, the paths scoped to the package's tree; set `landing: cherry-picking` in the dispatch block; on main `git cherry-pick -n <package>`. A conflict is resolved on main by the orchestrator, never by an agent on main; a conflict the orchestrator will not resolve is undone with `git cherry-pick --abort`, which puts main back as it was.
...
13. Set `landing: verified` in the dispatch block, then book the package in `plan.md` and rewrite the state file: the landing note, the usage row, the dispatch block cleared, the next package named. This is the last write before the commit, so the commit carries a state file that describes it.
