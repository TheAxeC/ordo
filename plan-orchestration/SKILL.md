...
dispatch:
  ...
  report: <path of the report file>        # Codex: the -o file; Claude: the agent's final message
  exit: <path of the exit file>            # Codex: written by the launch wrapper when the process ends
```

An orchestrator that finds this block on resumption checks the exit file and the process before anything else: a worker still running is waited for, a finished one is landed, a dead one (no exit file, no process) is reported to the user with what its logs hold, never silently relaunched.
...
9. Repair round: a miss that stays inside the brief (an acceptance item not built, a test not written, a convention not met) goes back to the same worker once, with the list of misses, in the same worktree; the round is counted in the usage row. A miss that would change the scope, a requirement, a public shape or an established decision goes to the user instead. After one repair round, what is still short is either fixed by the orchestrator at landing (counted as a fix) or reported to the user with the misses sorted by cause: the orchestrator's configuration, the brief, a rule nobody wrote, the worker.
10. Run the `verify` commands in the worktree.
11. Land it: in the worktree `git add -A <the tool's directory> && git commit -q -m wip`, the paths scoped to the package's tree; on main `git cherry-pick -n <package>`. A conflict is resolved on main by the orchestrator, never by an agent on main.
12. Run the `verify` commands again on main. A red check here is fixed on main when the fix stays inside the brief (a fix at landing, counted); otherwise the package is unstaged (`git checkout -- <paths>` for the tracked files, the package's new files removed by path) and goes back to step 9 with the failure.
13. Book the package in `plan.md` and rewrite the state file: the landing note, the usage row, the dispatch block cleared, the next package named. This is the last write before the commit, so the commit carries a state file that describes it.
14. Commit by explicit path, from `git diff --cached --name-only` plus the ledger files, in the user's message shape: a capitalised imperative subject, a blank line, `- Verb ...` bullets, the last bullet the plan booking. No attribution of any kind, and never push. After it, `git status --short` shows nothing of the package's; an unrelated untracked file of the user's is left where it was.
15. Remove the worktree and its branch. A worktree's branch is a local branch that exists only until this step; a diff kept for the record is saved as a patch file under `agents/reviews/`, never as a branch.
...
- The sandbox also caps open file descriptors low enough that a suite with many file watchers fails inside it with `EMFILE: too many open files, watch`, whatever the network setting. A Codex worker reports that row as NOT DONE with the output; the orchestrator's run of the `verify` commands from its own shell, in the worktree and again on main, is then the first full run, and the report is read with that in mind rather than as a failure of the code.
...
Per package, one row in the state file's table: the worker's harness and model, tokens (input, cached, output where the harness reports them), tool uses or completed items, duration, lines added and removed, whether the first report passed its bar, the repair rounds, and the number and kind of fixes at landing. Tokens and lines say what a package cost; the last three say what it was worth, and a change to the process, the harness or the model is judged on both, on the same briefs and checks, package by package.
