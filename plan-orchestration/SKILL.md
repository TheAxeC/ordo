...
The ledger's vocabulary is vendor-free: a harness is named where it matters (the worker's `harness:model`, the launch recipe), nowhere else. This skill and a plan's `spec.md` carry no history: a rule states the rule; a date, an incident or a ruling belongs in the ledger (`plan.md`'s rulings, the state file), which is the log. The same holds for the code a package writes: a comment says what and why, never which package, session or earlier bug it came from.
...
review: always               # always, or earned: the reviewer runs unless the worker's record earns the skip ("The review, earned" below).
workers_at_once: 1           # 1, or 2 when two packages whose briefs name disjoint paths may run side by side ("Two packages in flight" below).
```
...
With `workers_at_once: 2` the block is a list of two such entries, one per package in flight, and `dispatch: none` when nothing runs.
...
2. Pick the next package that nothing blocks. One package at a time, whatever the parallel list allows, unless the block sets `workers_at_once: 2` and the next two packages qualify under "Two packages in flight".
...
6. While it runs, do ledger work only: the next brief, the bookings, the usage table. Never dispatch a second worker beyond what `workers_at_once` allows, and never redo a package on another harness or launch the next package while the user has asked for a pause.
...
8. Reviewer stage, when the block names one and `review` calls for it on this package (always under `review: always`; under `review: earned` by the section "The review, earned"): dispatch the reviewer with the diff, the brief and the spec, asking for the acceptance items not met, the spec rules broken and the tests missing, as its report; the orchestrator saves that report into `agents/reviews/<package>.md` and reads it as one more lead before its own read. Its usage goes into the table under its own row.
9. Run the `verify` commands in the worktree.
10. Repair round: a miss found by the read, the reviewer or the checks that stays inside the brief (an acceptance item not built, a test not written or one that covers a stated rule only at its boundary, a convention not met, a red check, a judgment call the worker flagged as reversible that the orchestrator reverses) goes back to the same worker once, with the list of misses, in the same worktree; the round is counted in the usage row. The same worker means the same session, not a new dispatch: a Codex worker is resumed with `codex exec resume <session_id> -c 'sandbox_mode="workspace-write"' -c 'sandbox_workspace_write.network_access=true' -c 'model_reasoning_effort="<worker_effort>"' -m <model> -o <repair report> --json - < <repair prompt>`, run from inside the worktree's tool folder and detached the same way as the launch, since the resume form takes neither `-C` nor `-s` and the sandbox, the network setting and the effort go through `-c`; a native Claude worker is resumed through the runner's message tool on the agent id in `session_id`; a `claude -p` worker with `claude -p --resume <session_id>` and the flags of its launch. Before the resume, the round's prompt, event log, report, exit file and pid go into the dispatch block as `round: 1` and the `repair_` entries, committed as at step 5, so a resumption can tell a first run from a repair in flight. A resumed Codex session replays its whole transcript, so its input tokens exceed the first run's; the usage row records both runs. A miss that would change the scope, a requirement, a public shape or an established decision goes to the user instead. A repaired diff goes back through steps 7 to 9 in full: the whole diff read again, the reviewer again when one is named (under `review: earned`, only when the repair went beyond its list, by the same section), the `verify` commands again; nothing is booked on a repair that was not re-read and re-run. After one repair round, everything still short and inside the brief is fixed by the orchestrator at landing (counted as a fix, re-verified, and named in the landing note with its cause: the orchestrator's configuration, the brief, a rule nobody wrote, the worker). There is no reporting branch: a miss inside the brief is never handed to the user as a gap, a sharp edge or a later item, unless it is a stop under the Stops section below; the package is not booked until it is closed. Only a stop (a miss that would change the scope, a requirement, a public shape or an established decision) goes to the user, and it goes as a booked line in `plan.md` or the state file's open-items list, repeated in every report until the user rules on it: reported means booked and repeated until closed.
11. Land it, with the ledger's landing script when the ledger holds one (the section "The landing script" says what it is and does): from the repository root, `sh <ledger folder>/land.sh <package> <base> <runs directory>` (`--no-browser` only when the ledger says the browser check is deferred), after checking that none of the package's paths carries an unrelated change of the user's on main and setting `landing: cherry-picking` in the dispatch block. The script does the by-hand sequence of this step and the checks of step 12, prints the diff stat, the usage rows in the state file's format and the staged paths, and stops before the package commit, the plan tick and the state rewrite. By hand, which is what the script does: in the worktree, before every writing git command, wait for `.git/index.lock` to go and remove one older than a minute only when no git process is alive; `git add -A <the tool's directory> && git commit -q -m wip`, the paths scoped to the package's tree (a repair round adds a second such commit on the branch); `git checkout -b <package>-land main`; then `git cherry-pick <base>..<package>`, the whole range from the recorded base, so every landing applies the complete reviewed change and never only the last repair. A conflict is resolved in the worktree by the orchestrator, never by an agent and never on main; the script does not resolve one: it prints the conflicting paths, exits 2 and leaves the worktree on `<package>-land`. On main, after the same lock wait, `git cherry-pick -n main..<package>-land`, which leaves the complete change staged and uncommitted, one implementation commit per package.
12. Run the `verify` commands again on main. When the landing script ran, its zero exit is this step's pass and the checks are not run a second time. A red check here is fixed on main when the fix stays inside the brief (a fix at landing, counted, and the checks run again); otherwise the package is taken back out of main and goes back to step 10 with the failure. Taking it out means discarding what `cherry-pick -n` put into the index and the working tree: `git restore --staged --worktree -- <paths>` for the files it changed, and `git rm --cached -- <path>` plus deleting the file for each file it added. Both `restore` and `rm` are discarding commands on the ask list, so the orchestrator asks before running them, which is what the list is for; `git checkout -- <paths>` does not do this (it copies the index into the working tree and discards nothing). A pre-existing change of the user's is never among the paths.
...
Under `review: earned` the reviewer stage is decided per package from the ledger's rows and the diff, never from the worker's name. Three parts. First, what never changes: the `verify` commands in the worktree, the orchestrator's read of the whole diff, and the commands again on main; the bar a first report is measured against is that read plus those checks, so it exists with or without a reviewer. Second, the reviewer runs unless the worker has earned its skip: it runs on the worker's first package under this rule, when any of the worker's last three rows under it did not pass the bar with at most one fix at landing, and, whatever the record, when the package's brief touches a server module, the state layer or a wire shape; a failed bar puts the reviewer back for the worker's next three packages. Third, the second review after a repair runs only when the repair went beyond its list: the repair delta (the hunks the repair added or changed against the first diff) is compared with the misses the round was sent; a delta that changed only those places and is under about 200 lines gets the orchestrator's read of the delta and no second reviewer, and one that touched other places or is larger gets the second review. The repair round stays at most one. The rows record which branch each package took, so the rule is judged like any other process change: on fixes at landing and on edges found afterwards, per package, with and without the reviewer.

## Two packages in flight

With `workers_at_once: 2` the orchestrator, still one, may have two packages running at once, each through steps 3 to 15 on its own, under these rules: each brief lists the paths its package writes, and the two lists share no file and no view (a package that touches shared files, a configuration file or a rule file runs alone); each package has its own worktree, base, worker, reviewer, repair round and dispatch entry, and its stages run in sequence within it; the second is dispatched only after the first's dispatch commit, so its base holds the first's brief and block; the landings are one at a time, in the order the packages are verified, and the second lands on the head the first left, its landing branch cut from that head and its whole diff read again there; a path that both ranges changed, seen at `git diff --name-only <base>..<package>` before the second landing, means the second goes back through step 7 on the new head before it lands. A reviewer or a repair of one package while the other's worker runs is inside the two.
...
A ledger may hold `land.sh`, one command that does steps 11 and 12 by hand's sequence, so a landing costs the orchestrator a read of one block instead of a dozen commands; `templates/land.sh` and `templates/land.test.sh` are the files a plan copies into its ledger folder and adapts. It is POSIX sh, run from the repository root as `sh <ledger folder>/land.sh <package> <base> <runs directory> [--no-browser]`: `<package>` is the worktree's branch, `<base>` the commit the worktree was cut at, `<runs directory>` the folder holding the worker's and reviewer's run files (`events.jsonl`, `pid.txt`, `exit.txt`, and the `repair-*`, `review-*` and `review-*-2` twins). It does, in order: a preflight that the root is on `main` and the worktree on `<package>`; in the worktree, from inside it, `git add -A <tool directory>`, `git commit -q -m wip`, `git checkout -b <package>-land main`, `git cherry-pick <base>..<package>`; on main, `git cherry-pick -n main..<package>-land`; then the `verify` commands from the tool directory in their configured order, stopping at the first failure with the step's name and the command's output; then one booking block: the diff stat against the base, the worker row and the reviewer row summed from the run files' `turn.completed` usage and `item.completed` counts with the seconds between each pid and exit file's mtimes, in the state file's row format, and `git diff --cached --name-only`. Before every writing git command it waits for that repository's `.git/index.lock` and removes one older than a minute only when no git process is alive. Exit codes: 0 landed and checked with the block printed; 2 a cherry-pick conflict, its paths printed and the worktree left on `<package>-land` for the orchestrator; 3 the browser suite's port taken; 64 bad arguments; any other status is the failed command's own. It never commits on main, never pushes, never removes a worktree, never resolves a conflict, and never uses `merge`, `rebase`, `reset --hard`, `restore` or `git -C`. `land.test.sh` proves it on scratch repositories under `mktemp -d` (a clean landing with its staged paths and rows, and a conflict with its exit code and paths); git runs there only. Adapting the template to a plan is three edits, each marked `ADAPT` in the file: the tool directory, the check commands and their pass rules, and the harness and model names in the rows.
