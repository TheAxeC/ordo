...
4. Create the worktree yourself, at main's head, never through a runner's own isolation feature: `git worktree add -b <package> <worktree_root>/<package> HEAD`, then `git -C <worktree_root>/<package> sparse-checkout set <worktree_paths>` when the block names any, then the dependency install the project needs. The folder is a git concept and is the same whichever harness the worker runs in.
